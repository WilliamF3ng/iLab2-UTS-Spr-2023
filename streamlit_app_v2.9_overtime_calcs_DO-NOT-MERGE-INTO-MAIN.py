...
def main():
    ...
    if user_type == "Employee":
        ...
        if selected_award == "MA000003":
            ...
            st.write(translator.translate(
                ...
                "store managers, tradespersons (butchers, bakers, florists) and travel agents.", dest=target_lang).text)
        ...
        if df is not None:
            ...
            filtered_df = filtered_df[
                (filtered_df["employeeRateTypeCode"] == "AD") | (filtered_df["employeeRateTypeCode"] == "JN")]
            filtered_df_penalty = filtered_df_penalty[(filtered_df_penalty["employeeRateTypeCode"] == "AD") | (
            ...
            st.caption("Don't know your classification level? [Click here!](https://www.fairwork.gov.au/employment-conditions/awards/award-classifications)")
            ...
            filtered_df_penalty = filtered_df_penalty[~(filtered_df_penalty["clauseDescription"]
                                                        .str.contains("casino gaming employees", case=False) == True)]
            ...
            st.error(
                translator.translate("Oops something went wrong. Please retry again later.", dest=target_lang).text,
                icon="❌")
        ...
        with st.form("hour_entry_form"):
            ...
            day_data = {day: {"hours_worked": 0.0, "is_evening_shift": False, "is_overtime": False,"is_public_holiday": False} for day in
                        days_of_week}
            ...
            for index, day in enumerate(days_of_week):
                ...
                with col1:
                    hours = st.number_input(f"Enter hours worked on {day}, excluding breaks:", min_value=0.0, max_value=24.0, step=0.1)
                    day_data[day]["hours_worked"] = round(hours, 1)

                with col2:
                    day_data[day]["is_evening_shift"] = st.checkbox(f"Evening shift on {day}", help="Tick if your shift started or ended after 6pm")
            ...
            superannuation_rate = st.slider("Superannuation rate", min_value=11.0, max_value=15.0, value=11.0, step=0.5,
                                            format="%f%%")
            ...
            if submitted:
                ...
                st.write(f"You have worked {total_hours_worked:.1f} hours! Your total gross pay is ${total_pay:.2f}.")
                ...
                if days_with_more_4hs:
                    st.write(f"- :hamburger: On {', '.join(days_with_more_4hs)}, you should've been allowed to take a lunch break. "
                ...
                if days_with_evening_shifts:
                    st.write(f"- :crescent_moon: On {', '.join(days_with_evening_shifts)}, you worked evening shift(s). You may be entitled to additional pay.")
                if selected_award == "MA000003":
                    st.write("Your evening rate is 110% of your ordinary pay from Monday to Friday 10pm to midnight OR 115% from midnight to 6am.")
                elif selected_award == "MA000004":
                    st.write("Your evening rate is 125% of your ordinary pay from Monday to Friday after 6pm.")
                else:
                    st.write("Your evening rate is \$ 2.62 per hr extra from 7pm to midnight OR $ 3.93 per hr from midnight to 7am.")
        ...
        if employer_selected == "General Retail Industry Award (MA000004)":
            st.write("This pay award covers employers and employees working in the general retail industry. "
    ...
    if user_type == "Employee":
        ...
        with st.expander(translator.translate("I have been underpaid!", dest=target_lang).text):
            ...
            \n 6. If your pay hasn’t been corrected by the date decided, follow up with your boss and ask for an update.''', dest=target_lang).text)
        ...
        with st.expander(
                translator.translate("After speaking to my boss, I have been dismissed or threatened with dismissal!",
                                     dest=target_lang).text):
